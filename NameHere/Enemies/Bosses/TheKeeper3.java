...
public class TheKeeper3 extends FinalBoss {//stage 2 of finalBoss
    ...
    public int Attack(Player p, List<Enemy> allies) {
        ...
        if (allies.size() == 1) {
            try {
                allies.add(Main.allEnemies.get((Main.r.nextInt(Main.allEnemies.size()-1))).getClass().getConstructor().newInstance());
                allies.add(Main.allEnemies.get((Main.r.nextInt(Main.allEnemies.size()-1))).getClass().getConstructor().newInstance());
    ...
    public void finalBossOnSpawn(List<Enemy> enemies) {
        ...
        if (essay ==2) {
            ...
            Helper.Sleep(1000);
            System.out.println(Colors.RED + "The Keeper: " + Colors.RESET  + "And you, my dear, are just a mere player");
            Helper.Sleep(1000);
            Helper.contiuePrompt();
        }
        System.out.println(Colors.CLEAR);
        ...
        try {
            enemies.add(Main.allEnemies.get((Main.r.nextInt(Main.allEnemies.size()-1))).getClass().getConstructor().newInstance());
            enemies.add(Main.allEnemies.get((Main.r.nextInt(Main.allEnemies.size()-1))).getClass().getConstructor().newInstance());
        } catch (InstantiationException | IllegalAccessException | InvocationTargetException | NoSuchMethodException e) {
            throw new RuntimeException(e);
        }
